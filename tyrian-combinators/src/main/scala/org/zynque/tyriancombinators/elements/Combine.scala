...
def combineElements[F[_], I, O, T <: Tuple](
  ...
  new TyrianElement[F, I, O, CombinedMessage, StateTypes[T]] {
    ...
    def dispatchInput[T <: Tuple, I, O](
      ...
      val results =
        elements.productIterator.zip(statesIterator).map { case (e, s) =>
    ...
    def dispatchMessage[T <: Tuple, I, O](
      ...
      val element =
        elements
          ...
          .asInstanceOf[TyrianElement[
            F,
            I,
            O,
            ElementAt[T, F, I, O, index.type]#M,
            ElementAt[T, F, I, O, index.type]#S
          ]]
      val state = states
        .asInstanceOf[Tuple]
        .productElement(index)
      ...
      val statesArray =
        states.asInstanceOf[Tuple].toArray.asInstanceOf[Array[Any]]
